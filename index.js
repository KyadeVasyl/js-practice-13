...
function manipulateSearchParams(paramsObj, newUrl) {
  ...
  //та додаємо параметри пошуку до urlObj.
  for (let key of keys) {
...
function createURLWithParams(params, url) {
  ...
  const urlObj = new URL(url);
  // За допомогою циклу 'for in' перебираємо всі ключі
  //та значення об'єкта params та додаємо їх як параметри пошуку до urlObj.
...
/**
 * Функція `appendSearchParam` приймає URL-адресу, 
 * // ключ і значення та додає новий параметр пошуку до URL-адреси.
...
function processUrl(url, options) {
  ...
  if (options && options.searchParams) {
    for (let param in options.searchParams) {
      ...
      // Для кожного ключа параметру додаємо його та
      // відповідне значення до об'єкта 'urlObj' за допомогою методу 'append'.
...
function manipulateQuery(url, options) {
  ...
  if (options.get("append")) {
    ...
    for (let [key, value] of appendParam) {
...
function getUrlKeys(url) {
  ...
  const keysArr = urlObj.searchParams.keys();
